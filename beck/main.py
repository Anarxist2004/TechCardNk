...
from services.Changers.ch_IntensifyingScreenByVoltage import (
    IntensifyingScreenByVoltage,
)
from services.Changers.ch_ProtectiveScreenByVoltage import (
    ProtectiveScreenByVoltage,
)

sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8")

DB_DSN = "host=localhost port=5435 dbname=victor1 user=postgres password=1"
...
def create_pipeline(repos: PostgresDataBase) -> PipeLine:
    ...
    pipe_line.addChanger(RengenApparatusFromDb(repos, repos), 0)
    ...
    pipe_line.addChanger(RengenApparatusForPanoramicScheme(repos, repos), 0)
